...
const Home: NextPage = () => {
  ...
  // Knowledge
  const [knowledgeMaxShowContents, setKnowledgeMaxShowContents] = useState(4);
  const [knowledgeAddedlist, setKnowledgeAddedlist] = useState<hoge[]>(
    knowledgeTmplist.slice(0, 4)
  );
  const [knowledgeNewShowList, setKnowledgeNewShowList] = useState<hoge[]>([]);

  const loadMoreKnowledgeArticles = () => {
    setKnowledgeNewShowList(
      knowledgeTmplist.slice(
        knowledgeMaxShowContents,
        knowledgeMaxShowContents + 4
      )
    );
    setKnowledgeAddedlist([...knowledgeAddedlist, ...knowledgeNewShowList]);
    setMaxShowContents(maxShowContents + 4);
  };
  // const loadMoreKnowledgeArticles = () => {
  //   setNewKnowledgeShowList(
  //     knowledgeTmplist.slice(maxShowContents, maxShowContents + 4)
  //   );
  //   setAddedlist([...addedlist, ...newShowList]);
  //   setMaxShowContents(maxShowContents + 4);
  // };
    ...
    <Template>
      <>
        ...
        <div className="grid grid-cols-1 gap-[20px] h-min text-center">
          ...
          {knowledgeNewShowList.map((value, i) => {
            return (
              <>
                <div className="hidden sm:block">
                  <button
        ...
        {knowledgeAddedlist.length == knowledgeTmplist.length &&
        knowledgeNewShowList.length == 0 ? (
          <div className="my-[72px]"></div>
        ) : (
          <button
            onClick={() => {
